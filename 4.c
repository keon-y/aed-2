...
void removeNode(Node*, Node**, Node*, int*);
...
void rotateLeft(Node **pt) {
	...
	if (ptu->bal == 1) {
		...
		*pt = ptu;
		...
		(*pt)->dir = ptv->esq;
		ptv->esq = *pt;
		if (ptv && ptv->bal == 1) (*pt)->bal = -1; 
			else (*pt)->bal = 0;
		if (ptv && ptv->bal == -1) ptu->bal = 1;
			else ptu->bal = 0;
		*pt = ptv;
	}
	(*pt)->bal = 0;
...
void checkRotation(Node **pt, int plus) { //plus pra saber se adiciona 1 ou subtrai 1 do bal
	int bal = plus ? (*pt)->bal + 1 : (*pt)->bal - 1;
	if (bal == 2) rotateLeft(pt);
	if (bal == -2) rotateRight(pt);

}

//remove um no `node`
void removeNode(Node *node, Node **pt, Node *ancestor, int *h) {
	if (!node) return;
	if (node != *pt && node->chave < (*pt)->chave) { 
		removeNode(node, &(*pt)->esq, *pt, h);
		if (*h) {
			checkRotation(pt, 1);
		}
	}
	else if (node != *pt && node->chave > (*pt)->chave) {
		removeNode(node, &(*pt)->dir, *pt, h);
		if (*h) {
			checkRotation(pt, 0);
		}
	}
		...
		//caso d
			...
			Node *successor_ancestor = getAncestor(*pt	, successor);
